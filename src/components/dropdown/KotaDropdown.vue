...
const apiHelper = new ApiHelper('kota')
interface Option {
  label: string;
  value: any;
}
...
const fetchData = () => {
  apiHelper.fetchList((val) => loading.value = val, (res: any) => {
    ...
    if (route.query.kota_id) {
        const selectedOption = options.value.find(option => option.value == route.query.kota_id);
        if (selectedOption) {
...
watch(() => props.provId, (newValue) => {
    if (newValue) {
      options.value = dropdownValues.value.filter((x: any) => x.prov_id == newValue)
    }
});
